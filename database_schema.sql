...
--   OR
--   3. Use psql: psql -h <host> -U postgres -d postgres -f database_schema.sql
-- ============================================================
...
CREATE TABLE IF NOT EXISTS experience (
    ...
    end_date     TEXT        NOT NULL,                          -- e.g. "Present"
...
-- ============================================================
-- TABLE: profile
-- Stores the developer's About Me information.
-- ============================================================
CREATE TABLE IF NOT EXISTS profile (
    id           UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
    name         TEXT        NOT NULL DEFAULT '',               -- Developer full name
    tagline      TEXT        NOT NULL DEFAULT '',               -- Hero section tagline
    bio          TEXT        NOT NULL DEFAULT '',               -- Professional narrative (About)
    core_values  JSONB       NOT NULL DEFAULT '[]'::jsonb,      -- [{icon, title, description}]
    metrics      JSONB       NOT NULL DEFAULT '{}'::jsonb,      -- {apps_delivered, happy_clients, ...}
    ...
    tech_stacks  JSONB       DEFAULT '{}'::jsonb,               -- {Languages:[...], Frameworks:[...]}
...
-- Admin full access (requires authenticated Supabase service role key)
CREATE POLICY "Admin full access for projects"
    ON projects ALL TO authenticated USING (true);

CREATE POLICY "Admin full access for experience"
    ON experience ALL TO authenticated USING (true);

CREATE POLICY "Admin full access for profile"
    ON profile ALL TO authenticated USING (true);

CREATE POLICY "Admin full access for messages"
    ON messages ALL TO authenticated USING (true);

CREATE POLICY "Admin full access for reviews"
    ON reviews ALL TO authenticated USING (true);

CREATE POLICY "Admin full access for settings"
    ON settings ALL TO authenticated USING (true);

-- Allow anyone to submit a message via the contact form
CREATE POLICY "Public insert for messages"
    ON messages FOR INSERT WITH CHECK (true);

-- Allow visitors to submit reviews (goes to pending)
CREATE POLICY "Public insert for reviews"
    ON reviews FOR INSERT WITH CHECK (status = 'pending');
